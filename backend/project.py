...
@app.post("/books/{book_id}/issue")
def issue_book(book_id: int, username: str):
    db = SessionLocal()
    ...
    if book.reserved_quantity > 0:
        ...
        if not book.issued_date:
            book.issued_date = date.today().isoformat()
        db.commit()
        title = book.title
        db.close()
        return {"message": f"Book '{title}' has been issued."}
    else:
        title = book.title
        db.close()
        return {"error": "No reserved copies available to issue."}
...
def return_book(book_id: int, username: str):
    ...
    if book.issued_quantity > 0:
        ...
        title = book.title
        db.close()
        return {"error": "No issued copies available to return."}
...
def login(login_user: LoginUser):
    ...
    db.close()
from passlib.context import CryptContext
from backend.database import Base, engine, SessionLocal, Book, User, Reservation
from fastapi import FastAPI
from pydantic import BaseModel
from datetime import date
